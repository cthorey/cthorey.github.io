---
layout: postwithbib
title: "ResNet family of encoders"
published: true
bibliography: 2020-01-10-resnet.bib
authors: None
---

# **ResNet family of encoders**
...
4. **ResNeSt**<d-cite key="resnest"></d-cite> combines the idea of the paper above into one. 

## ResNet <d-cite key="resnet"></d-cite>

The ResNet architecture consists of a stack of a bottleneck block which consist of the element-wise summation of two pathways
...
As for ResNet, the ResNetXt block consists of an identified path way and G additional pathways. 
Each path 
...
This [post](https://blog.yani.ai/filter-group-tutorial/) contains some very good explanation about what group convolution is.

Here is a typical convolution operation. In particular, we convolve a feature map $$c_1 \times H \times W$$ with $$c_2$$ filters, each with the size $$c_1 \times h_f \times w_f$$. 
...
During group convolution - each group will convolve a feature map $$\frac{c_1}{g} \times H \times W$$ with a $$\frac{c_2}{g}$$ filters, each with the size $$\frac{c_1}{g} \times h_f \times w_f$$. Each group therefore outputs a $$\frac{c_2}{g} \times H \times W$$ group of features. We then concatenate all the feature groups to get $$c2$$. 
...
Specifically, they introduce another type of bock, the **csSE** block form channel and spatial Squeeze and Excitation.
